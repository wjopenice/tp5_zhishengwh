...
require_once dirname(__FILE__).'/buildermodel/AlipayTradePagePayContentBuilder.php';
    ...
    function build_order_nox(){
        return date('Ymd').substr(implode(array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
    }

    //商户订单号，商户网站订单系统中唯一订单号，必填
    $out_trade_no = build_order_nox(); //trim($_POST['WIDout_trade_no']);

    //订单名称，必填
    $subject = "测试";//trim($_POST['WIDsubject']);

    //付款金额，必填
    $total_amount = trim($_POST['pay_amount']);//trim($_POST['WIDtotal_amount']);

    //商品描述，可空
    $body = trim($_POST['body']);//"测试产品";// trim($_POST['WIDbody']);
